...
use utils::{decrypt_sector, encrypt_sector, extract_regions, generate_iv, is_encrypted};
...
pub fn decrypt(file_path: String, decryption_key: &str, thread_count: usize) -> io::Result<()> {
    ...
    let output_file_path = format!("{}_decrypted.iso", file_path.strip_suffix(".iso").unwrap_or(&file_path));
    ...
    chunks.par_bridge().for_each(|chunk| {
        ...
        chunk_data.par_chunks_mut(SECTOR_SIZE).enumerate().for_each(|(offset, sector_data)| {
            ...
            if is_encrypted(&regions, sector_index as u64) {
...
pub fn encrypt(file_path: String, decryption_key: &str, thread_count: usize) -> io::Result<()> {
    ...
    let reader = Arc::new(Mutex::new(BufReader::with_capacity(CHUNK_SIZE, input_file)));
    let output_file_path = format!("{}_encrypted.iso", file_path.strip_suffix(".iso").unwrap_or(&file_path));
    let output_file = Arc::new(Mutex::new(OpenOptions::new().write(true).create(true).open(&output_file_path)?));
    ...
    chunks.par_bridge().for_each(|chunk| {
        ...
        chunk_data.par_chunks_mut(SECTOR_SIZE).enumerate().for_each(|(offset, sector_data)| {
            ...
            if is_encrypted(&regions, sector_index as u64) {
                let iv = generate_iv(sector_index as u64);
                let mut cipher = Aes128CbcEnc::new(&key, &iv);
                encrypt_sector(&mut cipher, sector_data).unwrap();
            }
        });
    ...
    let elapsed = start_time.elapsed();
    info!("Encryption completed in {:.2} seconds.", elapsed.as_secs_f64());
    info!("Data written to {}", output_file_path);
